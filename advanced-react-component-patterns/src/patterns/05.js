 ...
 * Perphaps inspired by React's controlled form elements, control props allow users of your component to control the UI state via centain "control" props.
 */
import React, {
  useState,
  useLayoutEffect,
...
const MediumClap = ({
  ...
  const componentJustMounted = useRef(true); //the first time when runs, its true
  ...
  /**
   * When the state is controlled by us, we go ahead and call setState and pass it all the values
   * But if the component is controlled, we just want to go ahead and call the user defined function.
   */
  ...
  //When this is invoked, they should return the right state values and that should update whatever has been passed to a provider
  //and which is considered by all the children
...
const Usage = () => {
  const [state, setState] = useState(INITIAL_STATE); //Obtain the state of MediumClap
  const handleClap = () => {
