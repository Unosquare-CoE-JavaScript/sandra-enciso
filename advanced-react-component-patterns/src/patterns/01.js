 ...
 */
/* Takes a wrapped component and returns a new component that has the same functionality with an additional layer (animation)  */
const withClapAnimation = (WrappedComponent) => {
  class WithClapAnimation extends Component {
    //this handles animation logic
    animationTimeline = new mojs.Timeline(); //class property. This is the first default value of the animation timeline
    state = {
      animationTimeline: this.animationTimeline,
    };

    /* the animations being implemented here 
    that is because we will be reaching out to the elements and making some changes and we want it as soon as the component mounts*/
    componentDidMount() {
      const tlDuration = 300; //the timelapse duration
      const scaleButton = new mojs.Html({
        //animation properties
        el: "#clap", //element(s) to target
