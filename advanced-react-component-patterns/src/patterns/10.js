 ...
 * It's a lot easier to implement the state reducer pattern using useReducer as opposed to useState
 */
...
//our internal types
/**
 * The state reducer pattern let the user supply their own reducer
 */
const Usage = () => {
  ...
  const reducer = (state, action) => {
    ...
    if (action.type === useClapState.types.clap && isClappedTooMuch) {
      return state; //returns the actual state
    }
    return useClapState.reducer(state, action);
    // Call an internal reducer instead of this
    // switch (type) {
    //   case "clap":
    //     return {
    //       isClicked: true,
  ...
  const {
    ...
    reset,
    resetDep,
  } = useClapState(userInitialState, reducer); //when the state has been called, the user can pass an initial state
